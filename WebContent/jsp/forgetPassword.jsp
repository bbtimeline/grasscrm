	  ...
	  <div id="content">
		  <div id="header">
			...
			<div id="darkbanner" class="banner320">
				<h2><s:text name='login.forgetPassword.label'/></h2>
			</div>
			<div id="darkbannerwrap"></div>
			<s:form id="forgetPasswordForm" validate="true" namespace="/jsp/system" action="forgetPassword" method="post">	
				<fieldset class="form">
					<p>
						<label class="loginlabel" for="username"><s:text name='login.username.label'/>:</label>
							<s:textfield id="username" name="username" cssClass="logininput ui-keyboard-input ui-widget-content ui-corner-all" onkeypress="keypressTab();"/>
					</p>
					<p>
						<label class="loginlabel" for="email"> <s:text name='entity.email.label'/>:</label> <span>
						<s:textfield id="email" name="email" cssClass="logininput ui-keyboard-input ui-widget-content ui-corner-all" onkeypress="keypressTab();"/>
						</span>
					</p>				
					<button id="loginbtn" type="button" class="positive" name="Submit"
						type="submit" onclick="forgetPasswordForm.submit();">
						<img src="<s:url value="/images/key.png"/>" alt="Announcement" /><s:text name='submit.button.label'/>
					</button>
				</fieldset>
			</s:form>
			<div style="cursor: hand;text-align: right;">
			  <a href="<s:url value="/jsp/login.jsp"/>"><s:text name='login.loginpage.label'/></a>&nbsp;
			</div>
		</div>		
	</div>
